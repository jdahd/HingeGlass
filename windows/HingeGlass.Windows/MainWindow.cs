...
public sealed partial class MainWindow : Window
{
    ...
    readonly CheckBox follow=new(){Content="Follow lid angle",Foreground=Brushes.LightGray,IsEnabled=false,Margin=new Thickness(0,12,0,12)};
    ...
    public MainWindow()
    {
        Title="HingeGlass · Windows Preview 0.2.2";
        ...
        AddDesktopControls(controls);
        AddButton(controls,"Open image…",OpenImage);
        AddButton(controls,"Take desktop snapshot",async()=>await Snapshot());
        AddButton(controls,"Pause and restore",()=>RestoreDesktop("Restored"));
        controls.Children.Add(new TextBlock{Text="Desktop effect uses the selected display. Ctrl+Alt+Esc restores globally. Clicks pass through the visual effect at their original desktop coordinates.",TextWrapping=TextWrapping.Wrap,Foreground=Brushes.LightGray,Margin=new Thickness(0,18,0,0)});
        ...
        var note=new TextBlock{Text="Angle-driven perspective · blur · dimming\nLive desktop is opt-in. No screen frames are saved or uploaded.",TextWrapping=TextWrapping.Wrap,Foreground=Brushes.LightGray,Margin=new Thickness(0,12,0,0)};Grid.SetRow(note,1);right.Children.Add(note);
    ...
    void UpdateReading(double a)
    {
        ...
        monitor.Add(clock.Elapsed.TotalSeconds,a);lastAngle=a;samples++;follow.IsEnabled=true;
