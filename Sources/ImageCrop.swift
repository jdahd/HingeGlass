...
final class ImageCropEditor: NSObject {
    ...
    init(image:CGImage, size:CGSize, placement:ImagePlacement) {
        ...
        super.init()
        panel.title="Crop and resize image"; panel.isReleasedWhenClosed=false
        ...
        for (name,control) in [("Size",zoom),("Left / right",horizontal),("Down / up",vertical)] {
        ...
        let hint=NSTextField(labelWithString:"Proportions stay unchanged. Fit adds black borders where needed."); hint.font = .systemFont(ofSize:11); root.addArrangedSubview(hint)
        let reset=NSButton(title:"Reset",target:self,action:#selector(resetImage))
        let cancel=NSButton(title:"Cancel",target:self,action:#selector(cancel)); cancel.keyEquivalent="\u{1b}"
        let apply=NSButton(title:"Use image",target:self,action:#selector(apply)); apply.keyEquivalent="\r"
    ...
    @objc func cancel() { panel.sheetParent?.endSheet(panel); panel.orderOut(nil) }
    @objc func apply() {
